 ...
 */
export const buyTokenUsingETH = async (
  contract: ethers.Contract,
  collectionAddress: string,
  tokenId: number,
): Promise<string> => {
  try {
    const tx = await contract.buyTokenUsingETH(collectionAddress, tokenId)
 ...
 * @returns transaction hash, or null
 */
export const buyTokenUsingWETH = async (
  contract: ethers.Contract,
  collectionAddress: string,
  tokenId: number,
  ...
  try {
    const tx = await contract.buyTokenUsingWETH(collectionAddress, tokenId, price)
    const receipt = await tx.wait()
    return receipt.transactionHash
  } catch (error) {
    console.error(error)
