...
const BuyTicketsModal: React.FC<BuyTicketsModalProps> = ({ onDismiss }) => {
  ...
  const { balance: userCake, fetchStatus } = useTokenBalance(tokens.srs.address)
  ...
  useEffect(() => {
    const getMaxPossiblePurchase = () => {
      const maxBalancePurchase = memoisedUserCake.div(priceTicketInDogon)
  ...
  useEffect(() => {
    ...
    const costBeforeDiscount = priceTicketInDogon.times(numberOfTicketsToBuy)
  ...
  }, [ticketsToBuy, priceTicketInDogon, discountDivisor, getTicketCostAfterDiscount])
  ...
  const getErrorMessage = () => {
    if (userNotEnoughCake) return t('Insufficient SRS balance')
  ...
  return (
    <StyledModal title={t('Buy Tickets')} onDismiss={onDismiss} headerBackground={theme.colors.gradients.cardHeader}>
      ...
      <BalanceInput
        ...
        currencyValue={
          cakePriceBusd.gt(0) &&
          `~${ticketsToBuy ? getFullDisplayBalance(priceTicketInDogon.times(new BigNumber(ticketsToBuy))) : '0.00'} SRS`
        }
      />
      ...
      <Flex flexDirection="column">
        <Flex mb="8px" justifyContent="space-between">
          ...
          <Text color="textSubtle" fontSize="14px">
            {priceTicketInDogon && getFullDisplayBalance(priceTicketInDogon.times(ticketsToBuy || 0))} SRS
          </Text>
        </Flex>
        <Flex mb="8px" justifyContent="space-between">
          <Flex>
