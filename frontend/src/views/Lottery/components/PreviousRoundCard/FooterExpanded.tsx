...
const PreviousRoundCardFooter: React.FC<{ lotteryNodeData: LotteryRound; lotteryId: string }> = ({
  ...
  if (lotteryNodeData) {
    const { amountCollectedInDogon } = lotteryNodeData
    prizeInBusd = amountCollectedInDogon.times(cakePriceBusd)
  }
      ...
      <>
        ...
        {prizeInBusd.isNaN() ? (
          ...
          <Balance
            ...
            color="textSubtle"
            unit=" SRS"
            value={getBalanceNumber(lotteryNodeData?.amountCollectedInDogon)}
            decimals={0}
          />
        )}
      </>
