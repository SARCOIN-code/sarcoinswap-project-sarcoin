...
const LotteryCardContent = () => {
  ...
  const cakePrizesText = t('%cakePrizeInUsd% in SRS prizes this round', { cakePrizeInUsd: cakePriceBusdAsString })
  ...
  useEffect(() => {
    ...
    const fetchCurrentLotteryPrize = async () => {
      const { amountCollectedInDogon } = await fetchLottery(lotteryId)
      const prizeInBusd = cakePriceBusd.times(amountCollectedInDogon)
    ...
    <>
      <Flex flexDirection="column" mt="48px">
        ...
        <Text color="white" mb="40px">
          {t('Buy tickets with SRS, win SRS if your numbers match')}
        </Text>
      </Flex>
