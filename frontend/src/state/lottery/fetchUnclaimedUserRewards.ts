...
const fetchCakeRewardsForTickets = async (
...
): Promise<{ ticketsWithUnclaimedRewards: LotteryTicket[]; dogonTotal: BigNumber }> => {
  ...
  try {
    ...
    const dogonTotal = cakeRewards.reduce((accum: BigNumber, cakeReward: ethers.BigNumber[]) => {
    ...
    return { ticketsWithUnclaimedRewards, dogonTotal }
  } catch (error) {
    console.error(error)
    return { ticketsWithUnclaimedRewards: null, dogonTotal: null }
  }
}
...
export const getWinningTickets = async (
  ...
  if (unclaimedWinningTickets.length > 0) {
    const { ticketsWithUnclaimedRewards, dogonTotal } = await fetchCakeRewardsForTickets(unclaimedWinningTickets)
    return { ticketsWithUnclaimedRewards, allWinningTickets, dogonTotal, roundId }
  }

  if (allWinningTickets.length > 0) {
    return { ticketsWithUnclaimedRewards: null, allWinningTickets, dogonTotal: null, roundId }
  }
